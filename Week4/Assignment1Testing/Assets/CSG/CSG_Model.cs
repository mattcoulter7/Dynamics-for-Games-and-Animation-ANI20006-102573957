	...
	class CSG_Model
	{
		...
		public CSG_Model(GameObject go, bool reverseNormals, bool useCustomMaterial)
		{
			...
			if (useCustomMaterial == true)
			{
				...
				{
					vertices.Add( new CSG_Vertex(trans.TransformPoint(v[t[i]]), trans.TransformDirection(n[t[i]]), u == null ? Vector2.zero : u[t[i]], c == null || c.Length != vertexCount ? Color.white : c[t[i]], CSG_Model.submeshIndices.Count + 1));
			...
			else
			{
				for (int i = 0; i < m.subMeshCount; i++)
				{
					...
					for (int j = 0; j < submeshIndices.Length; j++)
					{
						vertices.Add( 
							new CSG_Vertex(
								trans.TransformPoint(v[submeshIndices[j]]), 
								trans.TransformDirection(n[submeshIndices[j]]), 
								u == null || u.Length != vertexCount ? Vector2.zero : u[submeshIndices[j]], 
